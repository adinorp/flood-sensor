...
error_t cipherEncryptData(CipherEngine *engine, uint8_t *data, size_t length)
{
   ...
   else if(engine->mode == CIPHER_MODE_CBC)
   {
      ...
      return cbcEncrypt(engine->algo, (void *) &engine->context,
...
error_t cipherDecryptData(CipherEngine *engine, uint8_t *data, size_t length)
{
   //Check parameters validity
   if(engine == NULL || data == NULL || length == 0)
      return ERROR_INVALID_PARAMETER;
   ...
   else if(engine->mode == CIPHER_MODE_CBC)
   {
      ...
      return cbcDecrypt(engine->algo, (void *) &engine->context,
         engine->iv, data, data, length);
   }
   else
   {

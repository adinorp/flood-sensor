...
//If "VTOR_OFFSET_ADDR" is defined in the preprocessors then an offset
// is added to the flash base address to take into consideration
// the bootloader that is located at the begining of the flash and
// the header + padding of the boot-able image that holds the application binary
// The value of the offset is calculated like this:
// offset = bootloader size (in terms of sector)
//    + image header size
//    + padding to match boot-able (modulo 1K) application binary addres
// offset = 32K (0x8000) + 64o + 0x3C0 = 0x8400

#ifdef VTOR_OFFSET_ADDR
